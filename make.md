...
我已经为您在这个项目的根目录下创建了一份

ubuntu_deploy.md
的部署文档。

您可以打开该文件查看详情。它详细介绍了无需 Docker，在 Ubuntu 上裸机部署的完整步骤。以下是核心思路的摘要：

环境安装：安装 Python 3.11+ 和 Node.js。我们推荐全局安装 yarn 和 pm2。
后端启动（端口 8001）：
进入
...
目录，创建虚拟环境并激活。
用 pip install 安装依赖。
退回项目根目录，使用 PM2 这个进程守护工具来运行并守护后端进程：
bash
pm2 start api/main.py --interpreter api/venv/bin/python --name deepwiki-api
前端启动（端口 3000）：
在项目根目录，用 yarn install 和 yarn build 构建。
同样用 PM2 把它常驻在后台：
bash
pm2 start yarn --name deepwiki-web -- run start
Nginx 反向代理配置：
安装 Nginx。
编写配置，让网关拦截 80 (HTTP) 或 443 端口。
把形如 http://你的IP/api/xxx 的流量路由给本地的 8001 后端。
把所有其他 http://你的IP/xxx 纯网页的流量路由给 3000 前端。
具体的依赖包参数和 Nginx 配置文件的完整代码，都写在

ubuntu_deploy.md
里面了，您可以直接复制使用。如果有遇到某一步报错，欢迎随时提问！
